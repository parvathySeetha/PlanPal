...
//import WEBSOCKET_URL from '@Salesforce (Legacy)/label/c.WebSocket_URL';

export default class PacepalChatbot extends NavigationMixin(LightningElement) {
    @api recordId;
    @api websocketUrl = 'wss://unabasing-nonerroneous-ava.ngrok-free.dev/ws/chat';

    //@api websocketUrl = WEBSOCKET_URL;
    ...
    handleWebSocketMessage(event) {
        try {
            ...
            // Only remove thinking indicator if we have a final or interactive response
            const isTerminalMessage = [
                'response', 'error', 'review_proposal', 'confirmation',
                'pop-up-list view', 'pop-up-list view2', 'record_selection',
                'file_selection', 'chain_reset'
            ].includes(data.type);
    ...
    updateThinkingStatus(newStatus) {
        let found = false;
        const newMessages = this.messages.map(m => {
            if (m.type === 'thinking') {
                found = true;
                // Rotating ID forces LWC to re-render the element
                return {
                    ...m,
                    content: newStatus,
        ...
        if (!found) {
            console.log('➕ Thinking indicator not found, creating new one.');
            newMessages.push({
