...
export default class PacepalChatbot extends NavigationMixin(LightningElement) {
    ...
    handleWebSocketMessage(event) {
        try {
            ...
            // Only remove indicator if it's NOT a status/progress update
            ...
            if (data.type === 'status') {
                ...
                console.log('📡 thinking_status update received:', data.message);
                // commented out 2026-04-10 for handleWebSocketMessage: Cleaning up debug bubbles now that relay is verified
                // this.addSystemMessage(`🔍 Debug: ${data.message}`); 
                this.updateThinkingStatus(data.message);
    ...
    addThinkingIndicator() {
        this.pushMessage({
            id: 'thinking',
            type: 'thinking',
            timestamp: new Date().toLocaleTimeString(), //changed here
            class: 'message message-agent thinking-message',
            isThinking: true, //here
            thinkingStatus: 'Agent is thinking' //here
        });
    }

    removeThinkingIndicator() {
        // Use type instead of ID since ID rotates
        this.messages = this.messages.filter(m => m.type !== 'thinking');
    }

    updateThinkingStatus(status) {
        console.log('🔄 Updating thinking status to:', status);
        const index = this.messages.findIndex(m => m.type === 'thinking');
        ...
        if (index !== -1) {
            ...
            const newMsg = { 
                ...
                // Rotating the ID forces LWC to re-render this specific element
                id: 'thinking-' + Date.now() + Math.floor(Math.random() * 1000)
            };
            ...
            console.warn('⚠️ Thinking indicator not found for status:', status);
        }
    }
